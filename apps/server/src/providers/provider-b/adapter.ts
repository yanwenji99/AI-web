...
import { providerBSelectors } from "./selectors.js";
...
export class ProviderBAdapter implements WebProvider {
  ...
  public async sendChat(request: ChatRequest): Promise<ChatResponse> {
    const mode = process.env.PROVIDER_B_MODE ?? "echo";
  ...
  private sendEcho(request: ChatRequest): ChatResponse {
    ...
    return {
      provider: this.id,
      model: request.model,
      text: `provider-b echo: ${prompt}`,
      error_code: null
    };
  }

  private async sendWeb(request: ChatRequest): Promise<ChatResponse> {
    const targetUrl = process.env.PROVIDER_B_URL;

    if (!targetUrl) {
      return {
        provider: this.id,
        model: request.model,
        text: "PROVIDER_B_URL is not configured",
    ...
    try {
      ...
      await page.waitForSelector(providerBSelectors.input, { timeout });
      await page.fill(providerBSelectors.input, prompt);

      const sendButton = page.locator(providerBSelectors.sendButton);
      if ((await sendButton.count()) > 0) {
        await sendButton.first().click();
      } else {
        await page.press(providerBSelectors.input, "Enter");
      }

      await page.waitForSelector(providerBSelectors.messageItem, { timeout });
      const text = (await page.locator(providerBSelectors.messageItem).last().innerText()).trim();
